  ...
  <div class="page-background">
    <main class="auth-container">
      <div class="auth-header">
        <h1>{{ isLogin ? t('auth.clientLogin') : t('auth.clientRegister') }}</h1>
        <p class="auth-subtitle">
          {{ isLogin ? t('auth.welcomeBackClient') : t('auth.createClientAccount') }}
        </p>
      </div>
        ...
        <template #content>
          <form @submit.prevent="handleSubmit" class="auth-form">
            <div v-if="!isLogin" class="field">
              <label for="name" class="input-label">{{ t('auth.fullName') }}</label>
              <pv-input-text
                  id="name"
      ...
      <div class="auth-footer">
        ...
        <div class="user-type-switch">
          <span>{{ t('auth.areYouTechnician') }}</span>
          <router-link to="/tech/auth" class="switch-link">
            {{ t('auth.switchToTechnician') }}
          </router-link>
        </div>
      </div>
    </main>
